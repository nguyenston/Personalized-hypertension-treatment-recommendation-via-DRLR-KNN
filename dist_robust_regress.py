...
def solve_drr(X, y, reg_l1, reg_l2):
    ...
    obj = (
        reg_l2 * cvx.norm([-beta, 1], 2)
        + reg_l1 * cvx.norm(beta, 1)
        + cvx.norm(X * beta - y, 1) / num_sample
    )
...
def _drr_loss_and_gradient(w, X, y, reg_l2, reg_l1):
    ...
    loss = np.mean(np.abs(sign_err)) + reg_l2 * beta_norm + reg_l1 * np.sum(np.abs(w))
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.solver == "scipy":
            ...
            res = optimize.minimize(
                _drr_loss_and_gradient,
                parameters,
                args=(arg_X, y, self.reg_l2, self.reg_l1),
                method="Nelder-Mead",
