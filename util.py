...
def build_validation_set_prescription(all_x, all_y, all_u, test_size=0.40):
...
def get_rf_impute_outcome(x_collection, y_collection, model_collections):
    ...
    for pres_id in range(num_prescription):
        # get core model
        outcome.append(model_collections["core_model"][pres_id].predict(all_x))
...
def return_prediction_and_std(input_data, model_collections):
    ...
    return (
        np.array(prediction_mean).T,
        np.array(prediction_std).T,
        np.array(prediction_outcome).T,
    )
...
def eval_prescription_probability(probability, impute_outcome):
    ...
    for p in range(num_prescription):
        prescription_outcome += np.array([
            item[p] * k for item, k in zip(impute_outcome, probability[:, p])
        ])

    return prescription_outcome


def find_prescription_threshold(
    pred_y, pred_y_std, previous, randomized, probability, prediction_outcome
):
    """
    ...
    if randomized == 1:
        threshold = np.sqrt(
            -2 * np.log(0.1 / num_prescription)
        )  # randomized strategy only
        T = previous - np.max(pred_y + threshold * pred_y_std, axis=1)
        T = np.maximum(T, 0)
        std_t = eval_prescription_probability(probability, prediction_outcome) < (
            previous - T
        )
    else:
